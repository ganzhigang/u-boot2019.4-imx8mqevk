...
#define USB_EN_PAD IMX_GPIO_NR(3, 5)
static iomux_v3_cfg_t const usb_en_pads[] = {
	IMX8MQ_PAD_NAND_CLE__GPIO3_IO5 | MUX_PAD_CTRL(NO_PAD_CTRL),
};

#define M2_EN_PAD IMX_GPIO_NR(3, 10)
static iomux_v3_cfg_t const m2_en_pads[] = {
	IMX8MQ_PAD_NAND_DATA04__GPIO3_IO10 | MUX_PAD_CTRL(NO_PAD_CTRL),
};

#define LTE_EN_PAD IMX_GPIO_NR(3, 7)
static iomux_v3_cfg_t const lte_en_pads[] = {
...
static void myir_baord_enable(void)
{
	imx_iomux_v3_setup_multiple_pads(usb_en_pads,
					 ARRAY_SIZE(usb_en_pads));
	gpio_request(IMX_GPIO_NR(3, 5), "usb_en");
	gpio_direction_output(IMX_GPIO_NR(3, 5), 0);
	udelay(10000);
	gpio_direction_output(IMX_GPIO_NR(3, 5), 1);

	imx_iomux_v3_setup_multiple_pads(m2_en_pads,
				 ARRAY_SIZE(m2_en_pads));
	gpio_request(IMX_GPIO_NR(3, 10), "m2_en");
	gpio_direction_output(IMX_GPIO_NR(3, 10), 0);
	udelay(10000);
	gpio_direction_output(IMX_GPIO_NR(3, 10), 1);

	imx_iomux_v3_setup_multiple_pads(lte_en_pads,
				 ARRAY_SIZE(lte_en_pads));
	gpio_request(IMX_GPIO_NR(3, 7), "lte_en");
